...
(function(){
  ...
  function renderImages(){
    document.querySelectorAll('img[data-img]').forEach(el=>{
      const value=(window.AIWIMG||{})[el.dataset.img];
      if(value) el.src=`data:image/webp;base64,${value}`;
    });

    const taskFig=document.querySelector('.task-hires');
    const task=(window.AIWIMG||{}).task;
    if(taskFig && task){
      taskFig.classList.add('task-focus-crop');
      taskFig.innerHTML=`<img alt="TASK-0023 Task Detail 실제 화면" src="data:image/webp;base64,${task}"><figcaption class="figcap">실제 Task Detail · Current checkpoint와 Latest handoff</figcaption>`;
    }
  }
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .concept-label{position:static!important;font-size:11.5px!important}
      .concept-badges{display:flex!important;align-items:center!important;gap:8px!important;flex-wrap:wrap!important;justify-content:flex-end!important}
      ...
      .concept-arrow{position:relative!important;z-index:1!important;color:#88a6ab!important}
